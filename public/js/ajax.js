    ...
    ajax.onreadystatechange = function() {
            ...
            if (ajax.readyState == 4 && ajax.status == 200) {
                ...
                for (let i = 0; i < respuesta.length; i++) {
                    ...
                    recarga += '<td>';
                    // editar
                    recarga += '<button class="edit" type="submit" value="Edit" onclick="abrirmodal_editar(' + respuesta[i].id_art + ',\'' + respuesta[i].nombre_art + '\',\'' + respuesta[i].precio_art + '\',\'' + respuesta[i].codigobarras_art + '\',\'' + respuesta[i].id_marca_fk + '\',\'' + respuesta[i].id_tipo_articulo_fk + '\');return false;">Editar</button>';
                    recarga += '</td>';
                    recarga += '<td>';
                    // eliminar
                    recarga += '<form method="post">';
                    recarga += '<input type="hidden" name="_method" value="DELETE" id="deleteNote">';
                    recarga += '<button class= "delete" type="submit" value="Delete" onclick="eliminar(' + respuesta[i].id_art + ');return false;">Eliminar</button>';
                    recarga += '</form>';
                    recarga += '</td>';
                    recarga += '</tr>';
                }
